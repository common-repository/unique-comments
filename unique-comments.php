...
Version: 0.2
Plugin URI: http://uplift.ru/projects/unique-comments/
...
function uc_add_menu() {
	add_options_page(__('Unique Comments', 'unique-comments'), __('Unique Comments', 'unique-comments'), 'administrator', __FILE__, 'uc_options_page');
}
add_action('admin_menu', 'uc_add_menu');

function uc_init() {
	load_plugin_textdomain('unique-comments', PLUGINDIR . '/' . dirname(plugin_basename(__FILE__)), dirname(plugin_basename(__FILE__)));
}
